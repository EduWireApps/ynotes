...
class CustomLogger {
  ...
  static void error(Object? e, {String? stackHint}) =>
      log("ERROR", e.toString(),
          stackTrace: ((stackHint != null)
              ? stackList[stackHint]["file_path"] +
                  ":" +
                  stackList[stackHint]["line"].toString()
              : ""));

  /// Logs a title and a message.
  static void log(String object, dynamic text,
      {String? stackTrace, bool save = true}) {
    if (save) {
      LogsManager.saveLogs(logs: [
        YLog(category: object, stacktrace: stackTrace, comment: text.toString())
      ], category: object);
    }
    debugPrint('[${object.toUpperCase()}' +
  ...
  static void logWrapped(String object, String description, String text,
    ...
    if (save) {
      LogsManager.saveLogs(
          logs: [YLog(category: object, comment: "$description: $text")],
          category: object);
    }
