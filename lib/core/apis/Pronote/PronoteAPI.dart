...
class PronoteClient {
  ...
  downloadUrl(Document document) {
    try {
      ...
      var magicStuff =
          this.encryption.aesEncryptFromString(conv.jsonEncode(data));
      String libelle =
          Uri.encodeComponent(Uri.encodeComponent(document.documentName!));
  ...
  homework(DateTime dateFrom, {DateTime? dateTo}) async {
    ...
    if (dateTo == null) {
      ...
      dateTo = f.parse(this.funcOptions['donneesSec']['donnees']['General']
          ['DerniereDate']['V']);
    }
    var jsonData = {
      'donnees': {
        'domaine': {
          '_T': 8,
          'V': "[${await getWeek(dateFrom)}..${await getWeek(dateTo)}]"
        }
      },
      '_Signature_': {'onglet': 88}
    };
    var response =
        await this.communication!.post("PageCahierDeTexte", data: jsonData);
    var jsonDataContenu = {
      'donnees': {
        'domaine': {'_T': 8, 'V': "[${1}..${62}]"}
      },
    ...
    var responseContent = await this
        .communication!
        .post("PageCahierDeTexte", data: jsonDataContenu);

    var cList =
        responseContent['donneesSec']['donnees']['ListeCahierDeTextes']['V'];
    //Content homework
    ...
    parsedHomeworkList.forEach((homework) {
      try {
        homework.sessionRawContent = listCHW
            .firstWhere((content) =>
                content.disciplineCode == homework.disciplineCode &&
                content.date == homework.date)
            .sessionRawContent;
      } catch (e) {}
    });
