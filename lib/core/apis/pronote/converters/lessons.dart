...
class PronoteLessonsConverter {
  static Lesson lesson(PronoteClient client, Map lessonData) {
    ...
    DateTime start = DateFormat("dd/MM/yyyy HH:mm:ss", "fr_FR")
        .parse(mapGet(lessonData, ["DateDuCours", "V"]));
    DateTime? end;

    int? endPlace = (mapGet(lessonData, ['place']) %
            ((client.funcOptions['donneesSec']['donnees']['General']
                        ['ListeHeuresFin']['V'])
                    .length -
                1)) +
    ...
    for (Map? endTime in (mapGet(client.funcOptions,
        ['donneesSec', 'donnees', 'General', 'ListeHeuresFin', 'V']))) {
      if (mapGet(endTime, ['G']) == endPlace) {
        ...
        DateTime endTimeDate =
            DateFormat("""HH'h'mm""").parse(mapGet(endTime, ["L"]));
        end = DateTime(start.year, start.month, start.day, endTimeDate.hour,
            endTimeDate.minute);
      }
    }
    ...
    String? room;
    try {
      var roomContainer = (mapGet(lessonData, ["ListeContenus", "V"]) ?? [])
          .firstWhere((element) => element["G"] == 17);
    ...
    //Some attributes
    String codeMatiere =
        mapGet(lessonData, ["ListeContenus", "V", 0, "L"]).hashCode.toString();
  ...
  static lessons(PronoteClient client, Map lessonsData) {
    ...
    List<Map> lessonsListRaw =
        (mapGet(lessonsData, ['donneesSec', 'donnees', 'ListeCours']) ?? [])
            .cast<Map>();
