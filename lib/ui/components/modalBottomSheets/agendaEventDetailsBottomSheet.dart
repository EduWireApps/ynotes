...
class _LessonDetailsDialogState extends State<LessonDetailsDialog> {
  ...
  void getAssociatedReminders() async {
    List<AgendaReminder>? remindersOnline =
        await appSys.offline.reminders.getReminders(widget.event.id);
    setState(() {
          ...
          children: <Widget>[
                      ...
                      child: FittedBox(
                        child: Text(
                          widget.event.name != "" && widget.event.name != null
                              ...
                              : "(sans nom)",
                          style: TextStyle(
                              fontFamily: "Asap", fontWeight: FontWeight.w700),
                            ...
                            child: Column(
                              ...
                              children: <Widget>[
                                ...
                                if (widget.event.location != null)
                                  buildKeyValuesInfo(
                                      context,
                                      widget.event.lesson != null
                                          ? "Salle"
                                          : "Emplacement",
                                      [widget.event.location]),
                                if (widget.event.lesson != null &&
                                ...
                                if (widget.event.lesson != null &&
                                    widget.event.lesson!.groups != null)
                                  SizedBox(
                                    height: (screenSize.size.height / 3) / 25,
                                  ),
                                if (widget.event.lesson != null &&
                                    widget.event.lesson!.groups != null)
                                  buildKeyValuesInfo(context, "Groupes",
                                      widget.event.lesson!.groups),
                                SizedBox(
                                  height: (screenSize.size.height / 3) / 25,
                                ),
                                if (widget.event.canceled! ||
                                    (widget.event.lesson != null &&
                                        widget.event.lesson!.status != null))
                                  buildKeyValuesInfo(context, "Statut", [
                                    widget.event.canceled!
                                        ? "Annulé"
                                        : widget.event.lesson!.status
                                  ]),
                                if (widget.event.description != null &&
                                    widget.event.description != "")
                                  buildKeyValuesInfo(context, "Description",
                                      [widget.event.description]),
                              ],
                            ),
                          ),
            ...
            Container(
              ...
              child: ListView.builder(
                  padding:
                      EdgeInsets.only(left: screenSize.size.width / 5 * 0.25),
                  ...
                  itemBuilder: (BuildContext context, index) {
                    if (index == reminders.length) {
                      return GestureDetector(
                        onTap: () async {
                          AgendaReminder? reminder = await (agendaEventEdit(
                                  context, false, lessonID: widget.event.id)
                          ...
                          if (reminder != null) {
                            ...
                            await AppNotification.scheduleReminders(
                                widget.event);
                          }
                        },
                        child: Card(
                            margin: EdgeInsets.only(
                                right: screenSize.size.width / 5 * 0.05),
                            color: Theme.of(context).primaryColorDark,
                            shape: RoundedRectangleBorder(
                                borderRadius: BorderRadius.circular(11)),
                            child: Container(
                                      ...
                                      child: RawMaterialButton(
                                        onPressed: () async {
                                          AgendaReminder? reminder =
                                              await (agendaEventEdit(
                                                      context, false,
                                                      lessonID: widget.event.id)
                                                  as Future<AgendaReminder?>);
                                          if (reminder != null) {
                                            setState(() {
                                              reminders.add(reminder);
                                            });
                                            appSys.offline.reminders
                              ...
                              child: FittedBox(
                                child: Wrap(
                                  ...
                                  children: [
                                    ...
                                    SizedBox(
                                        width: screenSize.size.width / 5 * 0.1),
                                    if (reminders[index].name != null &&
                                        reminders[index].name!.length > 0)
                                      Container(
                                        height:
