            ...
            Row(
              ...
              children: [
                RaisedButton(
                  ...
                  onPressed: () async {
                    String data =
                        await HiveBackUpManager(null).getBackUpFileData();
                    Map? decoded = jsonDecode(data);
                    if (decoded != null) {
                      //Try to import pinned homework
                      if (decoded["homework"] != null &&
                          decoded["homework"] != null) {
                        ...
                        await HiveBackUpManager(appSys.offline.offlineBox,
                                subBoxName: "homework", dataToImport: hw)
                            .import();
                      }
                      if (decoded["pinnedHomework"] != null &&
                          jsonDecode(decoded["pinnedHomework"]) != null) {
                        print("Importing pinned homework");

                        Map? dates = jsonDecode(decoded["pinnedHomework"]);
                        await HiveBackUpManager(
                                appSys.offline.pinnedHomeworkBox,
                                dataToImport: dates)
                            .import();
                      }

                      //Try to import lessons
                      if (decoded["lessons"] != null &&
                          jsonDecode(decoded["reminders"]) != null) {
                        print("Importing lessons");
                        Map? lessons = jsonDecode(decoded["lessons"]);
                        await HiveBackUpManager(
                                appSys.offline.pinnedHomeworkBox,
                                dataToImport: lessons,
                                subBoxName: "lessons")
                            .import();
                      }
                      //Try to import agenda events
                      if (decoded["agendaEvents"] != null &&
                          jsonDecode(decoded["agendaEvents"]) != null) {
                        print("Importing agenda events");
                        Map? agendaEvents = jsonDecode(decoded["agendaEvents"]);
                        await HiveBackUpManager(
                                appSys.offline.pinnedHomeworkBox,
                                dataToImport: agendaEvents,
                                subBoxName: "agendaEvents")
                            .import();
                      }
                      //Try to import reminders
                      if (decoded["reminders"] != null &&
                          jsonDecode(decoded["reminders"]) != null) {
                        ...
                        await HiveBackUpManager(appSys.offline.offlineBox,
                                subBoxName: "reminders",
                                dataToImport: reminders)
                            .import();
                      }
                    }
                  },
                  ...
                  onPressed: (exportPinnedHomework || exportEvents)
                      ? () async {
                          ...
                          if (exportPinnedHomework) {
                            json["pinnedHomework"] = HiveBackUpManager(
                                    appSys.offline.pinnedHomeworkBox)
                                .export();
                            json["homework"] = HiveBackUpManager(
                                    appSys.offline.offlineBox,
                                    subBoxName: "homework")
                                .export();
                          }

                          if (exportEvents) {
                            json["lessons"] = HiveBackUpManager(
                                    appSys.offline.offlineBox,
                                    subBoxName: "lessons")
                                .export();
                            json["agendaEvents"] = HiveBackUpManager(
                                    appSys.offline.offlineBox,
                                    subBoxName: "agendaEvents")
                                .export();
                            json["reminders"] = HiveBackUpManager(
                                    appSys.offline.offlineBox,
                                    subBoxName: "reminders")
                                .export();
                          }
                          //Write save file
                          await HiveBackUpManager(null)
                              .writeBackUpFile(jsonEncode(json));
                        }
                      : null,
                  child: Text(
