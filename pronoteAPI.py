...
class Client(object):
    ...
    def _login(self):
        ...
        ident_json = {
            ...
            "uuidAppliMobile": "",
            "loginTokenSAV": ""}
        idr = self.communication.post(
            "Identification", {'donnees': ident_json})
        ...
        # key gen
        if self.ent is True:
            motdepasse = SHA256.new(
                str(self.password).encode()).hexdigest().upper()
        ...
        # send
        auth_json = {"connexion": 0, "challenge": ch,
                     "espace": int(self.attributes['a'])}
        self.auth_response = self.communication.post(
            "Authentification", {'donnees': auth_json})
        if 'cle' in self.auth_response['donneesSec']['donnees']:
            self.communication.after_auth(
                self.communication.last_response, self.auth_response, e.aes_key)
    ...
    def lessons(self, date_from: datetime.date, date_to: datetime.date = None):
        ...
        if date_to:
            ...
            data['donnees']['dateFin'] = data['donnees']['DateFin'] = {
                "_T": 7, 'V': date_to.strftime('%d/%m/%Y 0:0:0')}
        output = []
...
class _Communication(object):
    ...
    def initialise(self):
        ...
        self.encryption.rsa_keys = {
            'MR': self.attributes['MR'], 'ER': self.attributes['ER']}
        uuid = base64.b64encode(self.encryption.rsa_encrypt(
            self.encryption.aes_iv_temp)).decode()
        # post
        json_post = {'Uuid': uuid}
        ...
        initial_response = self.post('FonctionParametres', {'donnees': json_post}, decryption_change={
                                     'iv': MD5.new(self.encryption.aes_iv_temp).digest()})
    ...
    def post(self, function_name: str, data: dict, recursive: bool = False, decryption_change=None):
        ...
        if self.compress_requests:
            ...
            log.debug(data)
            data = zlib.compress(data.encode(), level=6)[
                2:-4]  # actual compression
        ...
        r_number = self.encryption.aes_encrypt(
            str(self.request_number).encode()).hex()
        json = {'session': int(self.attributes['h']), 'numeroOrdre': r_number, 'nom': function_name,
                'donneesSec': data}
        p_site = self.root_site + '/appelfonction/' + \
            self.attributes['a'] + '/' + self.attributes['h'] + '/' + r_number
        ...
        if self.encrypt_requests:
            ...
            response_data['donneesSec'] = self.encryption.aes_decrypt(
        ...
        if self.compress_requests:
            ...
            response_data['donneesSec'] = zlib.decompress(
                response_data['donneesSec'], wbits=-15).decode()
        if type(response_data['donneesSec']) == str:
            try:
                ...
                raise PronoteAPIError(
                    "JSONDecodeError while requesting from pronote.")
    ...
    def after_auth(self, auth_response, data, auth_key):
        ...
        self.authorized_onglets = _prepare_onglets(
            data['donneesSec']['donnees']['listeOnglets'])
